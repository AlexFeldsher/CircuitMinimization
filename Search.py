...
def simulated_annealing(problem, schedule=exp_schedule()):
    ...
    for t in range(sys.maxsize):
        T = schedule(t)
        if T == 0:
            log(simulated_annealing, "T == 0")
            return current.state
        if current is not prev:
            neighbors = current.expand(problem)
        if not neighbors:
            log(simulated_annealing, "no neighbors")
            return current.state
        next_choice = random.choice(neighbors)
        delta_e = problem.value(next_choice.state) - problem.value(current.state)
        log(simulated_annealing, "current state", current.state.state)
        log(simulated_annealing, "next_choice", next_choice.state.state)
        log(simulated_annealing, "delta_e", delta_e)
        log(simulated_annealing, "T", T)
        log(simulated_annealing, "t", t)
        log(simulated_annealing, "probability", math.exp(delta_e / T))
        prev = current
