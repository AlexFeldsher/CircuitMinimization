...
def combine(low, high):
    ...
    if lowLen != highLen: raise Exception("Cannot combine implicants of different length")
...
def literalsToMinterms(pi):
    size = len(pi)
    minterms = list(range(2**size)) # all minterms are possible - now eliminate
...
def solvePetrick(epis, uncovered, coveredBy, numVars):
    ...
    closeCovers = [list(cover) for cover in product(*list(coveredBy.values()))]
    costs       = []
